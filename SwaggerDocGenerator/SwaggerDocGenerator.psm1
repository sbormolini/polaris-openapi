...
foreach ($item in $class) 
{
    try
    {
        Write-Verbose -Message "Importing class $($item.FullName)"
        . $item.FullName
    }
    catch 
    {
        Write-Error -Message "Failed to import class $($item.FullName):`n$($_)"
    }
}
...
foreach ($item in $functions) 
{
    try
    {
        Write-Verbose -Message "Importing function $($item.FullName)"
        . $item.FullName
    }
    catch 
    {
